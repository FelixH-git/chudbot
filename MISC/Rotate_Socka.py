...
class RobotLink:
    ...
    string payloads (e.g. 'shape,X,Y,angle') to it. Reused by the CV script too."""
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if shape in targets:
            ...
            # Optional angle input, for testing the RAPID-side rotation logic
            # without running the full vision pipeline. Blank = 0 deg.
            angle_str = input("Enter angle in deg (blank = 0): ").strip()
            try:
                angle_val = float(angle_str) if angle_str else 0.0
            except ValueError:
                print("Invalid angle, using 0.")
                angle_val = 0.0

            # Send format: "shape,X,Y,angle" - matches the payload format
            # produced by the vision script (test_bastler.py)
            payload = f"{shape},{x_val},{y_val},{angle_val}"
            sendToRobot(payload)
        else:
